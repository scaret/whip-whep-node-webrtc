...
const sendDataChannel = async ()=>{
    const message = document.getElementById('dataChannelMessage').value
    if (rtc.dataChannel){
        rtc.dataChannel.send(message)
    } else {
        console.error('rtc.dataChannel not exist')
    }
}
